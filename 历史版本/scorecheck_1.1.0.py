...
def get_personal_info(app, file_path):#file_path为单个文件的路径，遍历放在外层程序
    ...
    if not name or not number:
        ...
        if len(parts) >= 3 and parts[0] == "学年纪实测评表":
            ...
            number = None
            name = None
    PE_score= sheet.range("E36").value #体育成绩
    dom_score= sheet.range("E38").value #寝室分数，需要结合寝室成绩和是否寝室长
    is_level = sheet.range("E39").value #优秀寝室加分
    position_score = sheet.range("E32").value #班干部考核分数
    wb.close()
